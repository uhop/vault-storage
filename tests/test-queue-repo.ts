...
test('migration 0008 applies and queue_items is empty', t => {
  ...
  t.equal(row.value, '12', 'schema_version bumped to 12');
  db.close();
});

test('applyParsed — initial insert', t => {
  const {db, repo} = setup();
  const src = FM + ['## Backlog', '', '- **First.** body A', '- **Second.** body B', '', '## Watching', '', '- **Upstream.** waiting'].join('\n');
  ...
  t.deepEqual(open.map(r => [r.section, r.title]), [
    ['backlog', 'First.'],
    ['backlog', 'Second.'],
    ['watching', 'Upstream.']
  ]);
...
test('applyParsed — body change → updated, body identical but position shift → refreshed', t => {
  ...
  repo.applyParsed('demo', 'projects/demo/queue.md', parseQueueFile('demo', 'projects/demo/queue.md', v1), '2026-05-13T12:00:00Z');

  // v2 swaps order (position change for "Other") and rewrites "Same"'s body.
  const v2 = FM + ['## Backlog', '', '- **Other.** kept', '- **Same.** edited body'].join('\n');
  const result = repo.applyParsed('demo', 'projects/demo/queue.md', parseQueueFile('demo', 'projects/demo/queue.md', v2), '2026-05-14T00:00:00Z');
  ...
  t.equal(other?.updated_at, '2026-05-13T12:00:00Z', 'updated_at NOT bumped for placement-only refresh');
...
test('applyParsed — items removed from markdown are DELETEd', t => {
  ...
  repo.applyParsed('demo', 'projects/demo/queue.md', parseQueueFile('demo', 'projects/demo/queue.md', v1));
  t.equal(repo.count(), 3);

  const v2 = FM + ['## Backlog', '', '- **A.** a', '- **C.** c'].join('\n');
  const result = repo.applyParsed('demo', 'projects/demo/queue.md', parseQueueFile('demo', 'projects/demo/queue.md', v2));
  t.deepEqual(result, {inserted: 0, updated: 0, refreshed: 1, deleted: 1}, 'B deleted; C refreshes position 3→2');
...
test('applyParsed — rejects items from a different slice', t => {
  const {db, repo} = setup();
  const wrongSlice = parseQueueFile('other-project', 'projects/other/queue.md', FM + ['## Backlog', '', '- **X.** y'].join('\n'));
  t.throws(
...
test('deleteSlice removes everything under (project, source_file)', t => {
  ...
  repo.applyParsed('demo', 'projects/demo/queue.md', parseQueueFile('demo', 'projects/demo/queue.md', src));
  t.equal(repo.count(), 2);
...
test('listTopOpen — fleet-wide priority ordering', t => {
  ...
  const srcA = FM + [
    '## Backlog', '',
    '### Priority +2', '', '- **A-top.** a',
    '### Priority 0', '', '- **A-normal.** a'
  ].join('\n');
  const srcB = FM + [
    '## Backlog', '',
    '### Priority +1', '', '- **B-boosted.** b',
    '### Priority -1', '', '- **B-demoted.** b'
  ].join('\n');
  repo.applyParsed('alpha', 'projects/alpha/queue.md', parseQueueFile('alpha', 'projects/alpha/queue.md', srcA));
  repo.applyParsed('bravo', 'projects/bravo/queue.md', parseQueueFile('bravo', 'projects/bravo/queue.md', srcB));

  const top = repo.listTopOpen(10);
  t.deepEqual(top.map(r => [r.priority, r.project, r.title]), [
    [2, 'alpha', 'A-top.'],
    [1, 'bravo', 'B-boosted.'],
    [0, 'alpha', 'A-normal.'],
    [-1, 'bravo', 'B-demoted.']
...
test('listBySection / listByPriority — fleet-wide section + priority filters', t => {
  const {db, repo} = setup();
  repo.applyParsed('alpha', 'projects/alpha/queue.md', parseQueueFile('alpha', 'projects/alpha/queue.md', FM + [
    '## Active', '', '- **A-active.** a',
    '## Backlog', '',
    '### Priority +1', '', '- **A-boost.** a'
  ].join('\n')));
  repo.applyParsed('bravo', 'projects/bravo/queue.md', parseQueueFile('bravo', 'projects/bravo/queue.md', FM + [
    '## Active', '', '- **B-active.** b',
    '## Backlog', '',
    '### Priority +1', '', '- **B-boost.** b'
  ].join('\n')));

  const active = repo.listBySection('active');
  t.deepEqual(active.map(r => r.title), ['A-active.', 'B-active.']);

  const p1 = repo.listByPriority(1);
  t.deepEqual(p1.map(r => r.title), ['A-boost.', 'B-boost.']);
  db.close();
});
...
test('archive slice — listArchiveByProject orders by closed_at desc, NULLS last', t => {
  const {db, repo} = setup();
  const src = FM + [
    '## 2026-05-13', '', '- **Recent.** shipped',
    '## 2026-04-01', '', '- **Older.** shipped',
    '## Undated', '', '- **Undated item.** closed at some point'
  ].join('\n');
  repo.applyParsed('demo', 'projects/demo/queue-archive.md', parseQueueFile('demo', 'projects/demo/queue-archive.md', src));

  const archive = repo.listArchiveByProject('demo');
  t.deepEqual(archive.map(r => [r.closed_at, r.title]), [
    ['2026-05-13', 'Recent.'],
    ['2026-04-01', 'Older.'],
    [null, 'Undated item.']
  ]);
...
test('two slices share the same project (queue.md + queue-archive.md)', t => {
  const {db, repo} = setup();
  repo.applyParsed('demo', 'projects/demo/queue.md', parseQueueFile('demo', 'projects/demo/queue.md', FM + ['## Backlog', '', '- **Open.** ...'].join('\n')));
  repo.applyParsed('demo', 'projects/demo/queue-archive.md', parseQueueFile('demo', 'projects/demo/queue-archive.md', FM + ['## 2026-05-01', '', '- **Closed.** shipped'].join('\n')));
  t.equal(repo.count(), 2);
