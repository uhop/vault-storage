...
- **Closed enums** (`status`, `type`, edge types, suggestion kinds) are enforced by SQLite CHECK constraints — changing them is a schema migration, not an edit. Declaration surfaces may add _aliases_ instead (`EDGE_TYPE_ALIASES` in `src/records/types.ts`: `basis-for` → `derived-from` flipped) — normalized before storage, so the stored vocabulary and the CHECK stay closed.
