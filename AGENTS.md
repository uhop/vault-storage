...
- **`src/records/`**, **`src/queue/`** — record/edge repositories, reservation machinery (suggestion claims, repo leases), the queue-hygiene rules `/system/lint` runs (`queue/lint.ts`), the handoff queue (`handoffs.ts` + its spool file layer `handoff-spool.ts` — files under root `handoff/` are truth, the table is rebuilt by scan on server start), and the queue-items derivative synced from `queue.md` files.
