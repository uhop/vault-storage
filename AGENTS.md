...
- **Closed enums** (`status`, `type`, edge types, suggestion kinds) are enforced by SQLite CHECK constraints — changing them is a schema migration, not an edit. `agent.complexity` (`AGENT_COMPLEXITY` in `src/records/types.ts`) is closed at the write boundary in `src/server/writer.ts` instead — the block lives in frontmatter only — and so are the edge types in the FM `edges:` map and `agent.edge_classifications` (`DECLARED_EDGE_TYPES`). Declaration surfaces may add _aliases_ instead (`EDGE_TYPE_ALIASES` in `src/records/types.ts`: `basis-for` → `derived-from` flipped) — normalized before storage, so the stored vocabulary and the CHECK stay closed.
- **Schema migrations are append-only** numbered SQL files in `src/db/schema/`. Every applied migration forces a full import at the next start unless the file carries a `-- migrate:no-reindex` line; add it only when the migration changes nothing the importer derives from files (an index, an empty column). A migration that rebuilds the `records` table must drop + recreate + `'rebuild'` the FTS5 external-content index — its rowid linkage breaks silently otherwise.
- **Data-loss guards live at the storage boundary, never in a client.** The wipe guards (`empty_body`, `null_body`, `null_frontmatter_value`, `malformed_double_frontmatter`, `wrapped_frontmatter`) sit in `validateWritePayload` in `src/server/writer.ts`, so every caller — REST, MCP, UI, a hand-rolled `curl` — is covered by construction. A guard added to one client is a guard the other N clients don't have. Corollary when adding a write endpoint: give it a form whose blast radius is the thing being changed, not the whole document (`POST /vault/edit`, `PATCH /sections/{id}/fm` are the models).
...
- **`static/ui/`** — light dashboard/editor UI served by the server. Every page links `theme.css` before its own `<style>` and inherits the page chrome (reset, body and header frame, `main`, generic `button` with its `.primary`, `.danger`, and disabled states, `.err` / `.empty` / `.spinner` / `.footer`) from it; override a value by restating it, never by copying the block. The framework block at the end of the sheet holds the page-agnostic parts (`fmw-bar`, `fmw-table` with `fmw-scroll`, `fmw-tiles`, `fmw-records`, `fmw-card`, `fmw-pill`, `fmw-notice`, `fmw-eyebrow` with `fmw-count`, the `fmw-warn` / `fmw-bad` / `fmw-ok` / `fmw-quiet` ink, the link rule), which a page opts into with `class="fmw"` on `<body>`, the `vault-switch` element for a segmented control, and `fmw-sortable.js` for column sorting on an `fmw-table`, the page owning its column table and rendering; a part is one class on an island's root with its elements styled as elements, per the vault design note `projects/vault-storage/design/ui-css-component-system`. Verify a CSS change with `scripts/ui-style-snapshot.mjs` (snapshot before, snapshot after, diff).
...
- Operational scripts: `bin/` (update.sh, vault-curl); one-off DB probes and the release gates: `scripts/` (`ui-style-snapshot.mjs` for CSS changes, `ui-preview.mjs` to see a UI change on live data at any width, `mcp-smoke.mjs` for the adapter before a publish)
