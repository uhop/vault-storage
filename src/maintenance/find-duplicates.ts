...
/**
 * Default record types the scan ignores — the legitimately repetitive and
 * dated ones, for which "these two look alike" is the convention working
 * rather than a finding. `state` is autogenerated boilerplate (per the
 * vault-check-drift skill); `meta` is `_about.md` and similar; `log`,
 * `queue-item` and `index` are dated or structural series.
 *
 * `log` was added 2026-08-03 and is the measured one. Logs are 18% of the
 * corpus (310 of 1,728), transient by policy, and a merge between two dated
 * session logs is meaningless — yet log↔log pairs shared enough release and
 * session vocabulary to dominate every scan. A bounded live run produced 25
 * filed pairs and *all 25* were log↔log, the same class the 2026-05-13
 * incident recorded in the header. `type_mismatch` never damped them because
 * logs legitimately pair with logs.
 *
 * The set now matches `vault-lint`'s `TITLE_DUP_SKIP`
 * (`log`/`state`/`queue-item`/`index`/`meta`, "legitimately repetitive/dated").
 * Same question, same answer, previously answered in only one of the two
 * places — `queue-item` and `index` ride along on that rationale rather than
 * on measurement of their own.
 */
export const DEFAULT_SKIP_TYPES: readonly RecordType[] = [
  'state',
  'meta',
  'log',
  'queue-item',
