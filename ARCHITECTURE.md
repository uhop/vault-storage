...
- **`handoffs.ts`** — the handoff queue (agent coordination, legs 2–3): role-addressed work requests with an idempotent create, claim (lazy expiry), the D23 review-loop resolve (`done`/`rejected` archive into `projects/<project>/handoff-archive.md` and clear the spool entry; `returned` reopens for rework), resubmit, append-only notes, and the artifact pair (`PUT|GET /handoffs/{id}/artifact` — a 10 MB git patch or bundle, which sets the `spool` ref and travels with the sidecar). Unlike leases, handoffs survive restarts: the spool is truth, the table an index (see `src/records/handoff-spool.ts`).
- **`system.ts`**, **`lint.ts`** — status; integrity checks + queue hygiene (`queue_hygiene`, over `src/queue/lint.ts`) + enrichment-coverage block (see decisions D4/D5). `resume-bundle.ts` adds the one-shot `/vault resume` bundle (POST, embedded reindex) and its read-only brief tier (`GET /system/resume-brief`) for SessionStart-hook injection.
...
- **`src/tools.js`** — 63 tools (one per REST endpoint) with zod schemas mirroring the server's closed enums, at parity with the REST surface: reads, writes, lifecycle (`supersede`/`move`/`propose`), `/maintenance/*`, and the coordination families (`vault_lease_*`, `vault_handoff_*`), so the `/vault ingest`, `sweep`, `learn` and `resume` workflows all run MCP-native. Writes split by blast radius: `vault_append` / `vault_replace` / `vault_patch_fm` change only what they name (atomic server-side ops), while `vault_write_file` / `vault_update_piece` replace a whole document and take an optional `expected_etag`.
