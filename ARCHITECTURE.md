...
Vanilla-JS page set under `static/ui/` (public shell; API calls carry the user's bearer from localStorage): dashboard, search, note editor, folder/projects/tags/raw browsers, archive + lint review pages; shared `api.js`, theme, `vault-editor` / `vault-markdown` / `vault-toolbar` / `vault-nav` / `vault-settings` web components (the last two render the shared top-level menu and the ⚙ API-token dialog on every page), vendored `marked`.
