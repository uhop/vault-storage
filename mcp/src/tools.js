...
export const registerTools = (mcp, client) => {
  ...
  mcp.registerTool(
    ...
    {
      description:
        'One prepared context pack — replaces the search → similar → neighborhood → read chains. Anchor with exactly one of `query` (hybrid retrieval: lexical FTS + semantic chunk-KNN, fused by reciprocal rank) or `record_id` (that record’s nearest chunks). Returns {anchor, chunks, graph, budget}: chunks are [{record_id, file_path, title, chunk_index, text, semantic_score?, lexical_score?, sources}] — text is the bare body segment the stored vectors index, one best chunk per record; graph is rooted at the anchor (record mode) or the top fused record (query mode) and carries {root, neighborhood (1-hop records as agent.summary lines with typed edges + directions), neighborhood_total, backlinks, backlinks_total} — totals above the arrays’ length mean the 20-entry cap truncated them. The response is byte-budgeted (default 32 KiB, the MCP result ceiling’s safe zone): lowest-ranked chunks are dropped until it fits and budget reports {max_bytes, used_bytes, chunks_dropped} — never a silent truncation. Raise max_bytes explicitly when you want more.',
      inputSchema: {
