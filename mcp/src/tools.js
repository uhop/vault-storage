...
export const registerTools = (mcp, client) => {
  ...
  mcp.registerTool(
    ...
    {
      description:
        'Read a single suggestion with its full payload (classifier evidence, etc.). Returns the full suggestion row {id, kind, subject_id, status, payload, created, resolved_at, resolved_by, claimed_by, claimed_at, claim_expires}. Every payload carries evidence {source: vector | lexical | agent | metric | structural, asserted}: asserted is true only for a fact read from stored data (a stale hash, an unknown tag on frontmatter, an unreviewed link, a threshold on a count), false for anything a similarity threshold or an agent produced — read it before deciding how much to trust a row.',
  ...
  mcp.registerTool(
    ...
    {
      description:
        'File a new pending suggestion from the agent side. Use for kinds the indexer cannot deterministically detect — contradiction_candidate, agent-judged tag_suggestion, etc. No dedup at this layer; agent is responsible for any pre-check via vault_list_suggestions. Responds 201 with the created row {id, kind, subject_id, status, payload, created, …}. payload.evidence {source: vector | lexical | agent | metric | structural, asserted: boolean} is validated when present (400 invalid_enum_value otherwise) and defaults to {source: "agent", asserted: false}: a hand-filed finding is a judgement, not a stored fact.',
      inputSchema: {
        kind: SUGGESTION_KIND,
        subject_id: z.string().optional(),
  ...
  mcp.registerTool(
    ...
    {
      description:
        'Run integrity checks (bug-finding, not hygiene) over the vault DB. Ten categories: embedding_hash_drift, records_without_embeddings, orphan_embeddings, orphan_doc_embeddings, orphan_vec_rows, orphan_suggestions, temporal_anomalies (updated < created or future stamps), dangling_tag_aliases, auto_commit_failing, and queue_hygiene — every projects/*/queue.md against the project-queue convention (items under a non-schema H2, completion markers on open items, unbolded column-0 bullets, a heading glued to prose) plus the queue_items count against the markdown; its samples are {file_path, finding}. Returns {ok, total_issues, checks: {[name]: {count, samples}}, coverage} — samples are capped at 10 per check, and `coverage.enrichment` carries {total, enriched, unenriched, enrichable_types, by_type, unenriched_records}: the agent-enrichment coverage metric plus `unenriched_records`, a worklist of [{record_id, file_path, type}] that /vault sweep builds its backfill baseline from. Coverage is NOT an integrity check and never contributes to total_issues or flips ok. Cheap (~50ms on a few-thousand-record vault); safe to call from session-start flows like /vault resume. A clean result means no recorded gap, never proof of completeness — the coverage block counts what the index knows about.',
      inputSchema: {}
    },
  ...
  mcp.registerTool(
    ...
    {
      description:
        "Embed every record whose chunks or agent.summary vector are missing or stale — the fix for vault_lint's records_without_embeddings and embedding_hash_drift. A chunk or summary whose text did not change reuses its stored vector. Returns {embedded, upToDate, total, chunksWritten, chunksReused, docVecsWritten, summaryVecsWritten, remaining, durationMs}; the call loops rounds until the backlog is drained, so remaining is 0. A record whose vectors come back non-finite is stored incomplete and retried after a backoff (one minute, doubling to an hour), so embedding_hash_drift can still count it right after this call. Concurrent calls coalesce onto a single in-flight pass, so it is safe to fire more than once. Can run for seconds-to-minutes on a cold embedder (the ONNX pipeline reloads if it was released), unlike the microsecond cleanup ops.",
      inputSchema: {}
    },
