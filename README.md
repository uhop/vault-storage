...
**Status:** v0.x, in active development. Working: importer, embedder, REST server (full path-based vault surface + insight reads + suggestions queue + repo-lease registry), MCP adapter (53 tools / 3 resources), file-watcher with auto-reindex, edge GC, auto-commit, Docker packaging. The Obsidian cutover machinery (`/sync/from-obsidian`, mount plumbing) was retired 2026-06-11 after the migration completed; the one-time `migrate` CLI command remains for vault-tree imports. Not yet: suggestions review surface, decay/maintenance jobs.
...
Unknown query parameters are a 400 naming the offender and listing the supported set — a typo'd filter fails loud instead of falling through to an unfiltered answer. **Every endpoint enforces this**, including those that take no query parameters at all, which reject any; the sole exception is the `/ui/*` static surface, where browsers legitimately append `?v=<hash>` cache-busters. The guard runs before the handler's id lookup and before the request body is read, so a rejected call touches nothing — a bogus id still returns the parameter error, not a 404. On writes this is load-bearing rather than cosmetic: a typo'd `chek=true` on `PUT /vault/{path}` would otherwise leave the dedup gate disarmed while the caller believed it fired, and a typo'd `project_bodies` on `POST /system/resume-bundle` would silently return summaries where `/vault learn` needs verbatim bodies to dedup against.
...
| POST   | `/system/resume-bundle`         | One-shot session-start bundle for `/vault resume`: runs the incremental reindex, then packages lint (non-zero checks only), pending-suggestion counts by kind, the agent-workflow surface (Active section + clarify count), recent session-log `agent.summary` lines (`?logs=N`, default 3), and `?project=<name>`'s notes as summaries + sizes. `?project_bodies=feedback,queue,…` opts named project files into full-body delivery regardless of size (wrap prep). The feedback body is additionally included by default, but only while the whole bundle stays inside a 32 KiB budget — the MCP result channel rejects large results (observed just under ~50 KB), and feedback is the one body a caller receives without naming it; past the budget it carries `body_omitted: {reason: "bundle_budget", budget_bytes}` plus `headings`, a fence-masked section index, so the rules stay discoverable. Replaces the ~6 separate reads of the resume flow. POST because the embedded reindex writes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| GET    | `/system/resume-brief`          | The token-tiered sibling of the bundle, sized for SessionStart-hook injection (the `bd prime` pattern): a few hundred bytes answering "should this session run a full `/vault resume`?" — lint ok + issue count, pending-suggestion total, agent-workflow flags, `?project=<name>`'s Active item titles + backlog/ready/blocked counts, a feedback.md pointer (updated date — never the body), and the latest session log's title + date. Strictly read-only (no reindex, no claim reverts); titles, counts, and dates only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| GET    | `/sections`                     | List records. Filters: `type`, `status`, `tag` (CSV, alias-aware; unknown tags are a 400), `file_path` (alias: `path`), `file_prefix`, `priority_min/max`, `updated_since`, `record_ids`; plus `sort`, `exclude=body`. Pagination: `offset`, `limit` (max 100).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| GET    | `/vault/{path}`                 | Read a file (or compose an atomized folder back into one document). Responses carry an `ETag` of the served bytes — pair it with `If-Match` on PUT for safe read-modify-write. Composed responses (no on-disk file at the path) are marked `X-Vault-Composed: true` with a **weak** ETag (`W/"…"`) — they are virtual, so conditional writes against them always 412; edit the folder's pieces instead.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| PUT    | `/sections/{record_id}`         | Replace a record. **Programmatic callers use `Content-Type: application/json`** with `{frontmatter: {...}, body: "..."}` — the server serializes frontmatter to YAML itself, so there are no quoting traps. `text/markdown` (a `---\n<FM>\n---\n<body>` blob) is also accepted for human editing surfaces and verbatim round-trips of server-emitted files; never hand-author YAML through it. User keys merged; `created`/`updated` indexer-overridden; DB-only keys (`record_id`, `content_hash`, `last_referenced`, `decay_score`, `modified_at`) rejected. The merge is **union-only** — omitting a key keeps the stored value; to _remove_ a top-level key, send the reserved value `"__unset__"` (idempotent; below top level it 400s `nested_unset_sentinel`, since nested values are replaced wholesale — omit the key there instead). `agent.derived_from_hash: "auto"` / `agent.derived_at: "auto"` are finalized server-side (hash of the written body; current timestamp). Honors `If-Match` (see `PUT /vault/{path}`); the 204 carries the new `ETag`. Wipe guards (see `PUT /vault/{path}`) apply here too.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| PUT    | `/vault/{path}`                 | Create or replace a file. Same two modes and the same contract as `PUT /sections/{record_id}`: JSON `{frontmatter, body}` is the write path for agents and scripts; `text/markdown` exists for the UI editor and verbatim round-trips. Same downstream FM merge / enum validation / auto-managed-key rejection in both modes, including the `"__unset__"` key-removal sentinel (in markdown mode: `key: __unset__` in the FM block). **Optimistic concurrency (opt-in):** send `If-Match: <etag from GET>` and the write proceeds only if the document hasn't changed — otherwise 412 `precondition_failed` with `details.current_etag` for the re-read-merge-retry loop. `If-Match: *` requires the file to exist (conditional writes never create). No header → last-writer-wins, the original contract. The 204 carries the new `ETag`. **Atomized-folder shadow guard:** creating a file at a path whose folder twin is atomized (`X.md` next to `X/` with `_about.md`) 409s with `shadow_conflict` — the file would silently replace the composed view on GET; write pieces into the folder, or pass `?shadow=allow` to create it deliberately. A conditional PUT against such a path 412s with `details.composed: true`. **Wipe guards:** a body that is empty (400 `empty_body`) or the literal string `"null"` / `"undefined"` (400 `null_body`) is refused, as is a null top-level frontmatter value (400 `null_frontmatter_value`) — each is a caller interpolating a missing value over a real document, the class that wiped a 59 KB note on 2026-06-18. Removing a document is `DELETE`; changing one frontmatter key is `PATCH /sections/{id}/fm`. The empty-body rule exempts a document whose stored body is _already_ empty, so an FM-only op that round-trips the body is never bricked by a degenerate record. Guards live here rather than in any one client, so every caller gets them. |
| POST   | `/vault/edit`                   | Atomic server-side body edit — the read-modify-write primitive that retires the client GET → transform → ETag'd-PUT ceremony for running-file updates. Body, one op per call: `{path, op: "append", text}` or `{path, op: "replace", from, to, all?}`. Append collapses trailing whitespace to one newline before the fragment; replace is **asserted** — an absent `from` is a 409 `replace_assert_failed`, ambiguous without `all: true` is a 409 with the occurrence count — never a silent no-op. Frontmatter rides verbatim from disk (`updated` re-stamped; FM changes stay on PUT/PATCH); 404 on missing files (edit never creates); a composed atomized view 409s pointing at its pieces. No `If-Match` needed — the server holds the document. Returns `{path, etag, replaced?}`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| GET    | `/leases`                       | Repo-lease registry (agent coordination, D21/D23): who may edit which repo working tree directly. Flat `{count, items}`, unpaginated; `?resource=` filters to one lease — same shape, empty items = not held. Expired leases are lazily dropped on every read. `GET /leases/events` is the append-only transcript (newest first; cleared on server restart, like all coordination state).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| POST   | `/leases/claim`                 | Atomic claim: `{resource, holder, kind?, priority?, attestation?, ttl_seconds?}` → 200 `{status: claimed\|renewed\|preempted, lease}` or 409 `claimed_by_other` with the current lease. Re-claim by the holder is a renew (idempotent retry). Preemption lattice: human > cwd agent > side agent; nothing preempts a human holder, and human claims take no priority/ttl (never expire). Side claims attest a clean checkout (`attestation: "clean at <sha>"` — client-side check per D23). `POST /leases/renew`, `/leases/release` (`force` = operator hatch), `/leases/transfer` (atomic handover to `to_holder`; `to_kind: "human"` = review/commit request) complete the family — all validation-first, nothing mutated on rejection.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| POST   | `/suggestions/resolve-batch`    | Resolve up to 100 suggestions in one call, applying the **mechanical** side effects server-side: `tag_suggestion` accept realizes the tag on the record's FM (settles as `tag-realized` when the tag is in the taxonomy), reject strips the candidate from `agent.tags_suggested`; `edge_type` accept requires `edge_type` (typed value — "cites is correct" is a reject; the declared alias `basis-for` = forward derivation, stored as `derived-from` with the edge flipped) and pins the FM `edges:` override (settles as `fm-override`). Judgment-bearing kinds (`new_tag` minting, `duplicate` merging) resolve status-only. Body: `{resolved_by?, items: [{id, decision: "accept" \| "reject", edge_type?}]}`; `resolved_by` doubles as the claim holder. Always 200 — per-item failures land in `results[].error` with `accepted`/`rejected`/`failed` counts; one bad item never aborts the batch.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |

The table above is a selection. The full surface also covers search (`POST /search/simple`), the context pack (`POST /context-pack?query=…|record_id=…&k=N&max_bytes=N` — one prepared RAG pack: hybrid top-K chunks fused by reciprocal rank across the lexical FTS and semantic chunk-KNN legs, plus the anchor's deduped 1-hop typed neighborhood as `agent.summary` lines whose inbound entries are the backlinks; the whole response is byte-budgeted chunks-first — the neighborhood trims before any chunk drops, all reported — replacing the search → similar → neighborhood → read chains), edges (`/sections/{id}/neighborhood`, `/backlinks`, `/similar`), tags (`GET /tags`, `GET /tags/{tag}` — taxonomy row with description/aliases/count, `GET /tags/{tag}/records`, `POST /tags/taxonomy`, `POST /tags/aliases`), the suggestions review queue (`?expand=context` inlines per-item record briefs + tag info for triage; per-id `accept`/`reject`/`reopen` plus the batch claim/resolve pair above), queue-item slices (`/queue/*`, including the dependency-aware `GET /queue/ready` and `GET /queue/blocked` — Backlog items whose `blocked-by:` refs all resolve to archived items, and the complementary per-ref detail view with unresolved/ambiguous/cycle flags), maintenance ops (`POST /maintenance/*`), and the repo-lease registry (`GET /leases[?resource=…]`, `GET /leases/events`, `POST /leases/claim|renew|release|transfer` — agent coordination: atomic claim with a human > cwd-agent > side-agent precedence lattice, side claims attesting a clean checkout, TTL + lazy expiry, atomic transfer; coordination state is cleared on server start by design); the MCP adapter mirrors it as 53 tools.
...
- **MCP adapter** — the `mcp/` sub-package exposes the REST surface to Claude
  Code as 53 tools and 3 resources with closed-enum input schemas. Writes
  come in two shapes: the narrow ops (`vault_append`, `vault_replace`,
  `vault_patch_fm`) change only what they name and cannot lose the rest of
  a document, and the whole-document writers (`vault_write_file`,
  `vault_update_piece`) take an optional `expected_etag` for conditional
  writes, seeded by `vault_read_file{include_etag}`. Published
...
- **Closed enums.** `status` (5 values), `type` (14 values), edge types (10), suggestion kinds (8). Enforced by SQLite CHECK constraints. Declaration surfaces (FM `edges:` map, `edge_type` accepts) additionally take the alias `basis-for`, normalized at edge-build to `derived-from` with the direction flipped — never stored.
