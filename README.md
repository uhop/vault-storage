...
Unknown query parameters are a 400 naming the offender and listing the supported set — a typo'd filter fails loud instead of falling through to an unfiltered answer. Enforced across the whole `/vault/*` surface (reads, writes, `edit`, `move`, `supersede`, `propose`) and the record, edge, tag, suggestion, queue, search, context-pack and resume reads; an endpoint that takes no query parameters at all rejects every one. On `PUT /vault/{path}` this is load-bearing rather than cosmetic: a typo'd `chek=true` would otherwise leave the dedup gate disarmed while the caller believed it fired. Not yet on `GET /sections/{id}` and `/meta`, `/similar`, `/resolve`, `/queue/top` or `/maintenance/*`.
