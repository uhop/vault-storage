...
| GET    | `/system/lint`                  | Integrity checks (bug-finding): embedding hash drift, missing/orphan embeddings, temporal anomalies, dangling tag aliases. Returns `{ok, total_issues, checks, coverage}`. `coverage.enrichment` reports per-type `agent:`-block coverage (`{total, enriched, unenriched, by_type}`) over active records — a backfill metric, excluded from `ok`/`total_issues` since unenriched notes are normal, not bugs. ~50ms; safe on session-start flows.                                                                                                                                                                                                                                                                                    |
